...
class welcomeScreen:
    def __init__(self, window=None):
        ...
        window.title("Welcome to INDIAN BANK")
...
class customerMenu:
    def __init__(self, window=None):
        ...
        self.Button7 = tk.Button(self.Labelframe1, activebackground="#ececec", activeforeground="#000000",
                                 background="#39a9fc", borderwidth="0", disabledforeground="#a3a3a3",
                                 font="-family {Segoe UI} -size 9", foreground="#fffffe",
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0",
                                 text='''Fixed Deposit Calculator''', command=cal)
        self.Button7.place(relx=0.666, rely=0.855, height=24, width=181, bordermode='ignore')
        self.Button8  =tk.Button(self.Labelframe1, activebackground="#ececec", activeforeground="#000000",
                                 background="#39a9fc", borderwidth="0", disabledforeground="#a3a3a3",
                                 font="-family {Segoe UI} -size 9", foreground="#fffffe",
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0",
                                 text='''Recurring Deposit calculator''', command=rd_calci)
        self.Button8.place(relx=0.039, rely=0.855, height=24, width=181, bordermode='ignore')
...
def cal():
    ...
    def calcFD():
        ...
        r = (intRate.get())
        m = (intMon.get())
        res = float(p) * ((1 + (float(r) / 100) / intFreq) ** ((float(m) / 12) * intFreq))
        lblRes.config(text="Final Amount is %d" % round(res))
        return

    frPrincipal = Frame(master)
    frPrincipal.pack(fill=tk.X)
    lblPrincipal = Label(frPrincipal, text="Principal Amount")
    lblPrincipal.pack(side=LEFT)
    txtPrincipal = Entry(frPrincipal, textvariable=intPr, bd=1)
    txtPrincipal.pack(side=RIGHT)

    frInterest = Frame(master)
    frInterest.pack(fill=tk.X)
    lblInterest = Label(frInterest, text="Interest Rate per Year")
    lblInterest.pack(side=LEFT)
    txtInterest = Entry(frInterest, textvariable=intRate, bd=1)
    txtInterest.pack(side=RIGHT)

    frMonths = Frame(master)
    frMonths.pack(fill=tk.X)
    lblMonths = Label(frMonths, text="Number of Months")
    lblMonths.pack(side=LEFT)
    txtMonths = Entry(frMonths, textvariable=intMon, bd=1)
    txtMonths.pack(side=RIGHT)

    btnCalc = tk.Button(master, text="Calculate", command=calcFD)
    btnCalc.pack(side=BOTTOM)

    lblRes = tk.Label(master, text="")
    lblRes.pack()
...
def rd_calci():
    ...
    if __name__ == "__main__":
        rat = Tk()
        rat.configure(background='light green')

        rat.geometry("400x250")

        rat.title("Recurring Deposit Calculator")

        label1 = Label(rat, text="Principle Amount(Rs) : ",
                       fg='black', bg='red')

        label2 = Label(rat, text="Rate(%) : ",
                       fg='black', bg='red')

        label3 = Label(rat, text="Time(years) : ",
                       fg='black', bg='red')

        label4 = Label(rat, text="Compound Interest : ",
                       fg='black', bg='red')
        ...
        principle_field = Entry(rat)
        rate_field = Entry(rat)
        time_field = Entry(rat)
        compound_field = Entry(rat)
        ...
        button1 = Button(rat, text="Submit", bg="red",
                         fg="black", command=calculate_ci)

        button2 = Button(rat, text="Clear", bg="red",
